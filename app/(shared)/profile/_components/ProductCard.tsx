...
const ProductCard: FC<ProductCardProps> = ({ url, name, creator, id }) => {
  return (
    <Link href={`/drops/${id}`}>
      <article className="bg-dark-surface-200 flex h-full min-w-60 flex-col gap-4 overflow-hidden rounded-2xl shadow-2xl transition-transform hover:-translate-y-0.5">
        <figure>
          <Image
            src={url}
            alt={name}
